...
def register():
   if request.method == 'POST':
       ...
       elif len(email) <3 or len(email)>30 or email.count(space) !=0:
           user_error = 'Please enter a valid email address'
           password = ''
           verify = ''

       elif len(password) <3 or len(password)>20 or password.count(space) !=0:
       ...
       elif not user_error and not pass_error and not pass_verify:
...
def blog_entry_user():
   ...
   if 'id' in request.args:
       ...
       blogs = user.blogs
       return render_template('blog.html', author=author, user=user, blogs=blogs)
